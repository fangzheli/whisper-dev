...
from whisper.bzsp.types import NetworkState, Status, Bytes
...
async def test_permit_joining(bzsp_instance):
    async for bzsp in bzsp_instance:
        status = await bzsp.permit_joining(duration=t.uint8_t(60))
        asyncio.sleep(60) 
        assert status == Status.SUCCESS
...
async def test_send_aps_data(bzsp_instance):
    async for bzsp in bzsp_instance:
        ...
        status = await bzsp.send_aps_data(msg_type=t.uint8_t(0x00),
...
async def test_get_network_security_infos(bzsp_instance):
    async for bzsp in bzsp_instance:
        security_info = await bzsp.get_security_infos()
